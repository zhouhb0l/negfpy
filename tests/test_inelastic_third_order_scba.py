...
from negfpy.inelastic import FPUAlphaParams, fpu_alpha_scba_model, transmission_inelastic
from negfpy.models import analytic_band_max, device_perfect_chain, lead_blocks


def test_fpu_alpha_scba_zero_alpha_gives_zero_self_energy() -> None:
    params = FPUAlphaParams(mass=1.0, spring=1.0, alpha=0.0)
    model = fpu_alpha_scba_model(
        n_layers=8,
        params=params,
        temperature=0.3,
        broadening=0.02,
...
def test_fpu_alpha_scba_self_energy_is_finite_for_ballistic_green_function() -> None:
    params = FPUAlphaParams(mass=1.0, spring=1.0, alpha=0.06)
...
def test_fpu_alpha_scba_zero_alpha_matches_ballistic_transmission() -> None:
    params = FPUAlphaParams(mass=1.0, spring=1.0, alpha=0.0)
    lead = lead_blocks(params.harmonic_params)
...
def test_fpu_alpha_scba_reduces_toy_chain_transmission() -> None:
    params = FPUAlphaParams(mass=1.0, spring=1.0, alpha=0.06)
    lead = lead_blocks(params.harmonic_params)
    device = device_perfect_chain(n_layers=12, params=params.harmonic_params)
